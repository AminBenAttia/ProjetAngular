...
export class ChessBoardComponent {
  ...
  pieces: piecesType = [];
  piecesMap: Map<{ x: number; y: number }, ComponentRef<ChessPieceComponent>> =
    new Map();
  moveMode = false;
  ...
  mapGet(
    map: Map<{ x: number; y: number }, ComponentRef<ChessPieceComponent>>,
    mapKey: { x: number; y: number },
  ) {
  ...
  changeMapKey(
    map: Map<{ x: number; y: number }, ComponentRef<ChessPieceComponent>>,
    oldKey: { x: number; y: number },
    newKey: { x: number; y: number },
  ) {
    let piece: ComponentRef<ChessPieceComponent> | undefined;
    let newMap: Map<
      { x: number; y: number },
      ComponentRef<ChessPieceComponent>
    > = new Map();
    for (let [key, val] of map) {
    ...
    if (piece) newMap.set(newKey, piece);
    return newMap;
  }
  colorBlock(
    ...
    ctx.fillStyle = color;
    ctx.fillRect(
      y * this.size + padding / 2,
      x * this.size + padding / 2,
      this.size - padding,
      this.size - padding,
    );
  }
  ...
  removePiece(piece: ChessPieceComponent) {
    ...
    for (let p of this.pieces[piece.color == "b" ? 0 : 1]!.get(
      piece.pieceType,
    )!) {
      aux.push(p);
    }
    while (
      this.pieces[piece.color == "b" ? 0 : 1]!.get(piece.pieceType)!.length > 0
    ) {
    ...
    if (!this.isGameOver) {
      ...
      if (!this.moveMode) {
        ...
        if (this.selectedPiece) {
          let { greenBlocks, canKill } = this.selectedPiece!.canMove(
            this.pieces,
            this.turn,
          );
        ...
        if (this.selectedPiece) {
          ...
          if (this.killBlocks) {
            for (let block of this.killBlocks) {
              let piece = this.mapGet(this.piecesMap, {
                x: block.x,
                y: block.y,
              });
              if (piece) {
          ...
          if (canMove) {
            this.piecesMap = this.changeMapKey(
              this.piecesMap,
        ...
        let checkMate = check(
          this.pieces,
          this.pieces[this.turn == "b" ? 1 : 0]!.get("king")!.at(0)!.instance.x,
          this.pieces[this.turn == "b" ? 1 : 0]!.get("king")!.at(0)!.instance.y,
          this.turn == "b" ? "w" : "b",
        );
        if (checkMate) {
